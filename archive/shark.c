...
#define SHARK_VERSION "1.0.1"
#define DEBUG true		/* Set true for copious debugging output */
...
/*
	...
	ret = usb_claim_interface(hid->dev_handle, 2);
	if (ret) fprintf(stderr, "claim error %i %s\n", ret, usb_strerror());
*/
	ret = usb_interrupt_write(hid->dev_handle, WRITE_EP, (char*)PACKET, SEND_PACKET_LENGTH, 10000);
	if (ret) fprintf(stderr, "write error %i %s\n", ret, usb_strerror());
